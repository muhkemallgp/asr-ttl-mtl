...
# MODIFY get_encoding function (around line 270) - ADD include_diseases parameter
@lru_cache(maxsize=None)
def get_encoding(name: str = "gpt2", num_languages: int = 99, include_diseases: bool = False):
    ...
    # Languages + diseases
    base_languages = list(LANGUAGES.keys())[:num_languages]
    if include_diseases:
        disease_tokens = list(DISEASE_CONDITIONS.keys())
        all_languages = base_languages + disease_tokens
    else:
    ...
    specials = [
        "<|endoftext|>",
        "<|startoftranscript|>",
        *[f"<|{lang}|>" for lang in all_languages],
        "<|translate|>",
        "<|transcribe|>",
        "<|startoflm|>",
        "<|startofprev|>",
...
# MODIFY get_tokenizer function - ADD include_diseases parameter
@lru_cache(maxsize=None)
def get_tokenizer(
    multilingual: bool,
    *,
    ...
    if include_diseases:
        total_languages += len(DISEASE_CONDITIONS)

    encoding = get_encoding(
        name=encoding_name, 
        num_languages=total_languages,
